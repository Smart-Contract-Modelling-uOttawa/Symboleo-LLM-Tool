...
**`configs/ui_config.yaml`** — lives alongside pipeline run configs (same Docker volume mount). Holds model lists and parameter constraints (min/max/type — the form inputs read the bounds via `getParamConstraint`, with local fallbacks when a bound is absent, so editing one here reaches the UI). The temperature max is the cross-provider 0–2 envelope; per-provider caps are `compatibility.py` advisories, not UI bounds. Defaults come from Pydantic, not this file. Update without a code or frontend deploy.
