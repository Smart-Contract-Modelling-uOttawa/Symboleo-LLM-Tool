...
- **Integration tests:** Run the real JAR, either against the fixture files (`test_symboleo_cli.py` — valid `.symboleo`, invalid `.symboleo` with known errors; `test_valid_contract_returns_no_errors` asserts no **ERROR-severity** issues, benign unused-declaration WARNINGs allowed) or against contracts built inline from a shared valid `BASE` (the three fence files). Live LLM adapter tests optional/skippable in CI (`pytest -m "not live"`).
- **The three JAR fences catch different drift, and no one of them is sufficient.** A refresh can move a boundary in either direction, and each direction is invisible to the assertions that guard the other:
  ...
  - `test_ac_enforcement.py` — what the JAR must **reject**, i.e. the access-control `@Check` rules. Catches a refresh that *loses* a check. This is the direction nothing else can see: a jar enforcing less makes `valid.symboleo` validate clean, `invalid.symboleo` still fail on its typo, and the placement rules still hold — the suite gets greener, not redder.
...
- **No full e2e in CI** — manual smoke test before releases. `scripts/smoke_rejection.py` (`uv run python scripts/smoke_rejection.py`, needs Java 17) is the one that exists: it drives the real CLI → real JAR → writer → `report.json` and asserts against the artifacts on disk, faking only the LLM. **Faking the LLM is forced, not a shortcut** — the behaviour under test triggers on a model returning no contract, which cannot be provoked on demand, so the responses replay a real observed trajectory. The destructive trajectory is run twice, once with `_has_contract_span` patched off, so the output is an A/B against pre-fix behaviour rather than a green checkmark (the recovery trajectory has no control arm — its responses recover either way). That patch names a private symbol, which is acceptable because both decay modes are loud: a rename makes `patch.object` raise, and a gate bypassed at the call site fails the contrast checks. Add a script here per behaviour that only shows up across the whole stack — a new one should keep the same shape (real everything, faked only what is genuinely unprovokable, and a control arm).
...
2. **A ceiling, because this partial ships unconditionally.** `## Output Format` is capped at **15 top-level bullets**, asserted by `test_output_format_rule_count_is_capped` rather than narrated here, so the count cannot drift in prose. Reaching it means the next fix is few-shot or upstream message enrichment, **not** another bullet. The cap is a bullet count and not a token budget because the section's absolute size is stable while its share of a prompt is not — that ranges roughly 19–83% depending on strategy, `include_grammar`, and contract length (see Grammar Context Size), so no single share is a meaningful trigger. The failure mode it prevents is specific: unbounded accretion re-injects grammar knowledge into the `include_grammar: false` arm, converging the two arms until that comparison stops measuring what it names.
