...
### A contract-less correction response is adopted anyway, and can convert a good contract into a hollow "convergence"
`_run_candidate` assigns `code = _clean_response(...)` unconditionally, so whatever a correction returns becomes the next iteration's `current_code`. When a model gives up and returns something that is not a contract, the pipeline adopts it and the previous iteration's work is gone — a degenerate response is strictly worse than no response, because doing nothing would at least preserve the code.

**Observed 2026-07-31 (`output/run_20260731_104538/`).** Generation produced a strong 32-line contract — 4 obligations, 3 powers, correct `Suspended(obligations.<name>)` prefixes, `Assign(...)`, qualified enums — with exactly **one** error: two domain types named `Resumed` and `Terminated`, i.e. the reserved-keyword plateau (both *are* in `reserved_names()`; prevention is probabilistic and did not fire). At iteration 2 the model returned 23 tokens that were a garbled echo of the error message and no code at all. `_clean_response` handled it correctly by design (no span → passthrough), but the pipeline adopted it, so iteration 3 restarted from that garbage and produced an unrelated 13-line stub. The run reports `converged=True` for a final contract with **1 obligation and 0 powers** — no payment, late fee, suspension, or termination.

**Why this outranks a visible crash:** the run record looks like a success, so the damage is invisible to every number the pipeline reports and it inflates convergence rates (that batch is 4/5 genuine, not 5/5). It is also a second, independent way the *research artifact* can lie, alongside the masking failure `_clean_response` guards against.

**Fix direction (deferred).** In the correction loop, only adopt the cleaned response when it contains a `Domain`..`endContract` span; otherwise keep the previous iteration's code, record the rejected response in the `IterationRecord` (so the event stays visible in `report.json`), and let the next iteration retry. The span test already exists inside `_clean_response`, so the change is mostly about exposing that predicate rather than inventing one. Note the interaction with truncated output, which is deliberately *kept* today so the validator reports it — a truncated contract still has a `Domain` line and must stay adoptable.
