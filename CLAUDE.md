...
- **Coverage:** 116 tests, ~80% line coverage. Run with `uv run pytest --cov=symboleo_llm_tool --cov-report=term-missing`. Suite layer: `tests/unit/test_suite_runner.py` covers `run_suite()` against a mocked `pipeline.run`. Intentionally untested: `app.py` lifespan (integration-level) and `litellm_adapter.py` live LLM calls (token/cost extraction is covered in `test_litellm_usage.py` with a fake response).
