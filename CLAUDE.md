...
- **Integration tests:** Run the real JAR against known fixture files (valid `.symboleo`, invalid `.symboleo` with known errors). `test_valid_contract_returns_no_errors` asserts no **ERROR-severity** issues (benign unused-declaration WARNINGs are allowed — a valid AC contract can still emit them). `test_placement_rules.py` is the fence for the prompt's placement rules: each case is a whole contract differing from a shared valid `BASE` in one construct, and each illegal form is paired with the legal form the prompt prescribes — so a JAR refresh that moves a boundary fails here instead of leaving the prompt teaching a falsehood. Live LLM adapter tests optional/skippable in CI (`pytest -m "not live"`).
...
### Convergence rates are not comparable across two boundaries in the archive
`report.json` carries no schema version, so nothing inside an old artifact marks which rules produced it. Two changes moved what `converged` reports, in opposite directions:
