...
- **Correction only.** At iteration 0 there is no previous code to protect, so a contract-less generation is adopted and reported as a non-convergence. Gating there would require an empty-candidate state (`final_code=""`, `error_history=[]`) that nothing downstream handles.
...
- Each run directory contains: `report.json`, `config.yaml` (copy), `contract.txt` (the input contract — passed by both entry points; a suite dir carries one top-level copy beside `suite.yaml`, making it directly replayable), `contract_final.symboleo`, optionally `intermediates/`
...
### Convergence rates are not comparable across three boundaries in the archive
`report.json` carries no schema version, so nothing inside an old artifact marks which rules produced it. Three changes moved what `converged` reports, in different directions:
...
- **Placement rules** in `_output_format.j2` — *where* a construct may appear, as opposed to its form. Models write well-formed constructs in illegal positions: `Date.add` inside a comparison, a standalone `x: Date := …` variable, `Suspended(obligations.x)` as an *obligation* consequent, a bare `x := v` where `Assign(x := v)` is required, plus the article/enum/parameter-count/section-order traps. Each is JAR-verified and double-pinned per the policy below.
...
Thinking-capable models are already selectable (`configs/ui_config.yaml` lists `claude-opus-4-8`, `claude-sonnet-4-6`, `claude-haiku-4-5-20251001`), and they raise three concerns. The first (sampling-param rejection) is **handled**; the other two are still latent.
...
### A provider error mid-loop discards the whole run (Cohere makes this frequent)
An exception from `LLMAdapter.generate()` propagates out of `_run_candidate` → `pipeline.run()` → the caller, and the CLI writes its output directory only *after* `run()` returns. So a failure on any call after the first **destroys every completed iteration**: no `output/run_*/` directory, no `report.json`, no final code — while the tokens for that work are still billed. The console shows the progress lines for iterations that no longer exist anywhere (observed 2026-07-31: `Generated — 13 error(s), 8 warning(s)` followed by `litellm.Timeout: CohereException - Connection timed out after 120.0 seconds`, leaving 5 run directories for 7 attempted runs).

**Frequency is provider-specific and, for Cohere, high.** OpenAI has not failed this way; `command-a-03-2025` produced 2 timeouts in 4 runs on 2026-07-31, plus an earlier `NO_VALID_RESPONSE_GENERATED` (a Cohere-side empty response, same blast radius, different trigger). The 120 s bound in `_REQUEST_TIMEOUT_SECONDS` ([llm/litellm_adapter.py](symboleo_llm_tool/llm/litellm_adapter.py)) is not the bug and should not simply be raised — it was set against measured 6–12 s calls with ~50 s worst-case slowdowns, so a >120 s Cohere call is pathological rather than merely slow. Because `num_retries` is unset the bound is total wall-clock, which means one bad call kills a 5-iteration run that may be four iterations in.

**Cause is unconfirmed, but request size is ruled out.** Across the 5 completed Cohere runs, 17 successful calls span 6,741–10,434 prompt tokens and the *largest* (the few-shot generation call) succeeded in every run — so failures do not track payload. What remains fits contention: a heavy-tailed latency distribution (~8.5 s typical, a 54 s outlier, then >120 s), provider specificity, and a server-side empty-response variant. A plausible untested explanation is trial-key throttling; the distinguishing question is whether the limit is *rate* (spacing calls would help) or capacity (it would not), which Cohere's documented per-key rate limits would settle before spending credits on a latency probe.

**Why this matters beyond annoyance:** the artifact *is* the research output, and the losses are not random. A run is likelier to fail the longer it runs, so the surviving sample is biased toward short, easy runs — exactly the ones that converge — and any convergence rate computed over completed runs is optimistic by an unmeasured margin. Rerunning also can't recover the lost record, since generation is stochastic.

**Fix direction (deferred).** Catch provider exceptions at the candidate boundary and record a *failed candidate* — the iterations already completed, plus the error — instead of letting the run abort. That keeps partial data and makes the failure visible in `report.json` rather than only in a terminal the user has since closed. It is a `CandidateResult` shape change, so it carries the schema-regeneration obligation (`npm run generate-types`, committed with the change) and needs the API/SSE path to surface the same state. Retries (`num_retries`) are a separate, smaller lever and would change `_REQUEST_TIMEOUT_SECONDS` from a total bound to a per-attempt one — re-read that comment before touching it.
