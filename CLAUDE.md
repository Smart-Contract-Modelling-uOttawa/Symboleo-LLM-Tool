...
**Fix — staged, now at A.** **B (superseded):** labelling the grammar as an Xtext *definition, not example code* with a light "don't reproduce the notation" nudge. **B was confirmed insufficient:** three low-temperature (0.2) runs (`run_20260622_145716`/`_145908`/`_145946`) all produced `Date.add(effDate, delDueDateDays, 'days')` — the `'days'` leak survives B in 3/3 controlled samples, and the correction loop plateaus on it (the cryptic `"no viable alternative at input ''days''"` message doesn't encode the fix). **A (shipped):** a scoped Xtext-notation explainer in `_grammar_section.j2` that *supplies* the translation rules (single-quoted literals → emit without quotes; the `STRING` terminal → an actual quoted value; `ID` → a bare name; `CapitalizedNames` → rule names; `|`/`?`/`*`/`+`/`[Ref]` → notation). It is **scoped to notation** (Xtext-level, stable) not **rules** (grammar-specific, grows with the language), so it does not become a per-error caveat list; ~150 tokens, still grammar-agnostic and zero-drift. Because it lives in the shared grammar partial, both generation and correction inherit it — attacking the leak at the source *and* giving the correction loop the interpretive knowledge the raw error message lacks. The deepest fix — example-based grounding (few-shot), which contains zero meta-notation — is intentionally *not* used here: the leak is a cross-cutting grammar-layer defect (fix once, for all strategies, in the shared partial), and forcing few-shot to fix it would both contaminate the strategy comparison and cost ~1,000+ tokens per call.
