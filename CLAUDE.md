...
This is distinct from **Grammar Context Size** above: that is a token-budget concern; this is *format confusion*. The `## Output Format` module suppressed the leaks wherever it gives clean guidance (structure, obligations, powers); leaks persisted for **leaf constructs** it didn't cover (atomic function/literal forms like `Date.add`/`Date(...)`/`TimeUnit`), where the model falls back to misreading the raw grammar. Leaf defects come in two kinds — *notation* (quoting, rule-name reproduction) and *form* (wrong call shape/arity) — fixed respectively by the notation explainer and by per-construct `## Output Format` anchors (see Fix below).

**Fix — notation leak resolved at A; the form gap it exposed is now anchored.** The fix staged B → A. **B (superseded):** labelling the grammar as an Xtext *definition, not example code* with a light "don't reproduce the notation" nudge — confirmed insufficient (3/3 low-temp runs `run_20260622_145716`/`_145908`/`_145946` still produced `Date.add(effDate, delDueDateDays, 'days')`; the correction loop plateaus on the cryptic `"no viable alternative at input ''days''"`). **A (shipped):** a scoped Xtext-notation explainer in `_grammar_section.j2` *supplying* the translation rules (single-quoted literals → emit without quotes; the `STRING` terminal → an actual quoted value; `ID` → a bare name; `CapitalizedNames` → rule names; `|`/`?`/`*`/`+`/`[Ref]` → notation). Scoped to **notation** (Xtext-level, stable), not **rules** (grammar-specific), so it stays grammar-agnostic and zero-drift (~150 tokens); in the shared partial, so generation and correction both inherit it. **A is confirmed for the notation leak:** three later low-temp runs (`run_20260622_152924`/`_153000`/`_153031`) no longer emit `'days'` at all — the quote leak is gone in generation.

**Form regression A exposed (anchored; validation pending).** Freed from quoting, generation stopped emitting the near-correct `Date.add(effDate, delDueDateDays, 'days')` (one edit from valid) and instead invented `Date(effDate) + delDueDateDays days` — structurally wrong and unrecoverable (3/3, three errors each: the `Date(...)` literal expects a `STRING`, so `Date(effDate)` reports `mismatched input 'effDate' expecting RULE_STRING` and crashes the validator with `"Range [6, 3) out of bounds"`; and there is no `+ N days` infix). Root cause: **A supplies notation (de-quoting), not structural form.** `Date.add(arg, value, timeUnit)` ([Symboleo.xtext](symboleo_llm_tool/resources/Symboleo.xtext) lines 219–220; canonical use in `Contracts/Symboleo/MeatSale.symboleo:45`) is a *leaf construct* `## Output Format` didn't cover, so the model improvised the form. Fix: a positive form anchor for `Date.add(date, amount, timeUnit)` with a concrete bare-unit example added to `_output_format.j2` — a structural *rule* (grammar-specific), so it belongs in `## Output Format` alongside the other structural rules, **not** in the notation explainer (deliberately notation-scoped). Its convergence effect is pending the next live runs.

**Leaf-construct policy (altitude tripwire).** The `Date.add` anchor is a targeted n=1 fix. Do **not** add a second per-construct bullet if another leaf form breaks — that turns `## Output Format` into a hand-maintained grammar mirror (the per-construct caveat list we refused to let the notation explainer become). Rule of three: one occurrence → targeted bullet; a second/third → generalize. The generic escalation is (a) a **grammar-derived construct-signature reference** — the *presentation* sibling of A (A fixed how the model *reads* Xtext notation; this pre-translates the leaf `FunctionCall`/literal productions into clean surface signatures, generic and zero-drift if auto-derived from the grammar), or (b) **few-shot** demonstration (deepest and most generic, but a *strategy*-level lever kept out of the shared baseline to preserve the strategy comparison, and ~1,000+ tokens/call).

**Why not message-enrichment for the plateau.** The `'days'` plateau (and now the `Date(...)` form) raised whether the JAR's cryptic *messages* — `"no viable alternative at input ''days''"`, the validator's `"Range [6, 3) out of bounds"` crash, linking oscillations — are the bottleneck. A controlled single-correction probe (same stuck output/strategy/model/temp/issue-location, varying **only** the error message: raw JAR diagnostic vs an enriched actionable one) found **both converge 3/3 under the A build** — the raw cryptic message also fixes it. The probe is *confounded* (A's explainer is in both arms), so it cannot isolate message quality; what it *does* establish is that **A's grounding, not message text, resolved the `'days'` plateau** (grounding supplies the interpretive key the message lacks). Message quality remains a plausible *independent* limiter for errors A doesn't ground (the validator crash, linking errors), testable only on a build *without* the explainer. Deferred; and if pursued, the better fix is **upstream** (Symboleo's own Xtext `ISyntaxErrorMessageProvider` for syntax errors, `@Check` validator messages/quick-fixes) than a downstream enrichment layer, since we consume the JAR as a black box — feasible only if we can contribute to SymboleoAC-IDE, and a research-design choice (improving the feedback channel changes what the correction-loop experiment measures).

**Methodology note (temperature):** the temp-0.7 default produced wildly different generations run-to-run (failure modes barely overlapping), making prompt-effect attribution impossible. Lowering the OpenAI config to temp 0.2 ([configs/openai.yaml](configs/openai.yaml)) collapsed the variance — 3/3 runs became near-identical — which is what made the B-insufficient finding conclusive. Run prompt comparisons at low temperature; high-temperature single samples measure sampling noise, not the prompt.
...
- **Sampling params 400 — handled.** Opus 4.8/4.7, Fable 5, and the OpenAI reasoning models **reject** `temperature`/`top_p`/`top_k` (and `budget_tokens`) with a 400. The fix is layered:
  ...
  3. **Advisory warning:** [llm/compatibility.py](symboleo_llm_tool/llm/compatibility.py) flags a temperature set on a reasoning model so a silently-dropped/ignored param doesn't mislead — `reasoning_param_warnings()` is the per-`LLMConfig` primitive; `pipeline_param_warnings()` is the per-`PipelineConfig` aggregator that labels each stage (`generation:`/`correction:`) and is the single thing the CLI and API call. It uses `litellm.supports_reasoning` — empirically the right signal (correct for the modern reasoning models *including* the #26444 Opus cases, with zero false alarms), **not** `get_supported_openai_params` (wrong for current reasoning models in pinned LiteLLM versions, and false-alarms on Sonnet 3.5). Best-effort and never fatal — an unknown model yields no warning rather than a false alarm. Surfaced by the CLI (printed) and the API (`warnings` on `RunCreatedResponse`). `max_tokens` is intentionally not warned/dropped — LiteLLM translates it to `max_completion_tokens` for reasoning models.
